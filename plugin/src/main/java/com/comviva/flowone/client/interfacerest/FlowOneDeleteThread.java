...
public class FlowOneDeleteThread implements Runnable {
	...
	public FlowOneDeleteThread(CommandExecutor executor, GeneralBodyRequest grRq, int retries, DBLogger asaLogs,
			String endpointAddress, String imsi, String msisdn) {
	...
	private Integer sentRequest(String endpoint) {
		...
		try {
			...
			throw new KpiCommonTimeOutException(KPIS.DeletetePrepaidRSA + "Timeout endpoint: "+endpoint);						
		} catch (Exception e) {
	...
	private String createRequest() {
		...
		String request = "<soap:Envelope xmlns:soap=\"http://www.w3.org/2003/05/soap-envelope\" xmlns:ins=\"http://soa.comptel.com/2011/02/instantlink\">\n" //
				...
				+ "			<ins:Parameter name=\"SO1_ACTIONCODE\" value=\"DELETE\"/>\n"
	...
	@Override
	public void run() {
		MeasureUtil.measureWithException(KPIS.DeletetePrepaidRSA.getName(), measure->{
		this.currentTimeString = this.format.format(new Date());
		...
		new Thread(new DBLoggerThread(this.asaLogs, "FlowOne", "Delete create_prepaid_hrd", this.request,
		...
		new Thread(new DBLoggerThread(this.asaLogs, "FlowOne", "Delete create_prepaid_hrd", this.response,
				"1", "log", "", parameters.containsKey("sessionID") ? parameters.get("sessionID") : "-", msisdn, imsi,
						parameters.containsKey("iccid") ? parameters.get("iccid") : "-", "")).start();
